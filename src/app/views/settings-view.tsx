...
function ChatHistoryBudgetRow({ value, onChange }: BudgetRowProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (draftN !== value) void onChange(draftN)
    }, DEBOUNCE_MS)
  ...
  return (
    <section className="border-border bg-surface rounded-md border p-4">
      ...
      <Description>
        How many user+assistant pairs the chat service replays to the model on each send. Full keeps
        every prior turn; limited caps the most-recent N pairs to keep prompts small.
      </Description>
      <div className="mt-3 flex flex-col gap-3">
        ...
        <RadioOption
          ...
          onSelect={() => {
            void onChange(Math.max(1, draftN))
          }}
          label="Last N pairs"
          hint={
            mode === 'limited'
              ? `Currently capped at ${value ?? draftN} pair(s).`
              : 'Cap recent history.'
          }
        >
          {mode === 'limited' && (
...
function RadioOption({ checked, onSelect, label, hint, children }: RadioOptionProps): JSX.Element {
  return (
    ...
    >
      <input type="radio" checked={checked} onChange={onSelect} className="accent-text-brand" />
