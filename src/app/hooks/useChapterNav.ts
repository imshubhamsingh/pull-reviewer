...
export function useChapterNav(chapters: TourChapter[], opts: NavOptions): ChapterNav {
  ...
  useEffect(() => { setGlobalIdx((i) => Math.min(Math.max(0, i), Math.max(0, flat.length - 1))) }, [flat.length])

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (isTypingTarget(e.target)) return
      const handled = match(e.key)
        .with('ArrowLeft', () => { setGlobalIdx((i) => Math.max(0, i - 1)); return true })
        .with('ArrowRight', () => { setGlobalIdx((i) => Math.min(flat.length - 1, i + 1)); return true })
        .with('Home', () => { setGlobalIdx(0); return true })
        .with('End', () => { setGlobalIdx(Math.max(0, flat.length - 1)); return true })
        .with('ArrowUp', () => { setGlobalIdx((i) => jumpChapter(flat, i, -1)); return true })
        .with('ArrowDown', () => { setGlobalIdx((i) => jumpChapter(flat, i, +1)); return true })
        .with('r', 'R', () => { opts.onRegenerate(); return true })
        .with('Escape', () => { opts.onEscape(); return true })
        .otherwise(() => false)
