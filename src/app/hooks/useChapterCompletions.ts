 ...
 */
export function useChapterCompletions(repo: string, prNumber: number, headRefOid: string): ChapterCompletionsState {
  ...
  const toggle = useCallback(async (chapterId: string) => {
    ...
    setCompleted((prev) => {
      ...
      if (wasComplete) next.delete(chapterId); else next.add(chapterId)
      return next
    })
    try {
      if (wasComplete) await api.reviewProgress.chapters.unmark(repo, prNumber, headRefOid, chapterId)
      else await api.reviewProgress.chapters.mark(repo, prNumber, headRefOid, chapterId)
    } catch (e) {
      // Roll back
      setCompleted((prev) => {
        const next = new Set(prev)
        if (wasComplete) next.add(chapterId); else next.delete(chapterId)
        return next
      })
      setError((e as Error).message)
    }
