...
export function useChats(repo: string, prNumber: number): ChatsState {
  ...
  const send = useCallback(async (message: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: tempUserId, chatId, role: 'user', body: trimmed, references: null, status: 'complete', model: null, createdAt: now },
      { id: tempAssistantId, chatId, role: 'assistant', body: '', references: null, status: 'streaming', model: null, createdAt: now },
    ])
    setStreaming(true)
    setStreamEvents([])
    setError(undefined)
    ...
    try {
      for await (const evt of api.chats.sendStream(repo, prNumber, chatId, { message: trimmed }, { signal: ac.signal })) {
        ...
        if (evt.event === 'done') {
          ...
          setMessages((prev) => prev.map((m) => m.id === tempAssistantId
            ? { ...m, body: evt.data.message, status: 'error' }
            : m))
      ...
      if (ac.signal.aborted) {
        setMessages((prev) => prev.map((m) => m.id === tempAssistantId && m.status === 'streaming'
          ? { ...m, status: 'interrupted' }
          : m))
      } else {
        setMessages((prev) => prev.map((m) => m.id === tempAssistantId
          ? { ...m, body: message, status: 'error' }
          : m))
        setError(message)
      }
    } finally {
  ...
  const cancel = useCallback(() => { cancelStream(abortRef) }, [])
