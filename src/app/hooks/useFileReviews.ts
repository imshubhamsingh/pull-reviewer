 ...
 */
export function useFileReviews(repo: string, prNumber: number, headRefOid: string): FileReviewsState {
  ...
  const toggle = useCallback(async (filePath: string) => {
    ...
    setReviewed((prev) => {
      ...
      if (wasReviewed) next.delete(filePath); else next.add(filePath)
      return next
    })
    try {
      ...
      setReviewed((prev) => {
        const next = new Set(prev)
        if (wasReviewed) next.add(filePath); else next.delete(filePath)
        return next
      })
      setError((e as Error).message)
    }
  ...
  const markMany = useCallback(async (filePaths: string[]) => {
