 ...
 */
export function ReviewSummaryModal({ drafts, repo, sha, submitting, error, onCancel, onSubmit }: Props): JSX.Element {
  ...
  return (
    ...
    >
      <div className="border-border bg-bg flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-lg border shadow-xl">
        <Header count={drafts.length} fileCount={grouped.length} onCancel={onCancel} disabled={submitting} />
...
function SummarySection({ summary, onChange, disabled }: { summary: string; onChange: (s: string) => void; disabled: boolean }): JSX.Element {
  return (
    <div className="border-border bg-surface shrink-0 border-t px-4 py-3">
      <label className="text-text-muted mb-1 block text-[10px] tracking-wider uppercase">
        Review summary <span className="text-text-muted/60 normal-case tracking-normal">(optional — top-level review comment)</span>
      </label>
      <textarea
        value={summary}
...
function EventSelector({ event, onChange, disabled }: { event: ReviewEvent; onChange: (e: ReviewEvent) => void; disabled: boolean }): JSX.Element {
  return (
    <div className="border-border bg-surface shrink-0 flex items-center gap-2 border-t px-4 py-2">
      ...
      <EventOption value="COMMENT" current={event} onSelect={onChange} disabled={disabled} label="Comment" hint="Generic feedback, no approval/changes action" />
      <EventOption value="REQUEST_CHANGES" current={event} onSelect={onChange} disabled={disabled} label="Request changes" hint="Blocks merge until addressed" />
      <EventOption value="APPROVE" current={event} onSelect={onChange} disabled={disabled} label="Approve" hint="Approve the PR" />
    </div>
  )
}
...
function EventOption({ value, current, onSelect, disabled, label, hint }: EventOptionProps): JSX.Element {
...
function Header({ count, fileCount, onCancel, disabled }: { count: number; fileCount: number; onCancel: () => void; disabled: boolean }): JSX.Element {
  return (
    <div className="border-border flex shrink-0 items-center justify-between border-b px-4 py-3">
      <div>
        <h2 className="text-text-primary text-sm font-semibold">Submit review</h2>
        <p className="text-text-muted mt-0.5 text-xs">
          {count} {count === 1 ? 'comment' : 'comments'} on {fileCount} {fileCount === 1 ? 'file' : 'files'}
        </p>
      </div>
      <button
        type="button"
...
function FileGroup({ file, drafts, repo, sha }: { file: string; drafts: ReviewDraft[]; repo: string; sha: string }): JSX.Element {
  ...
  return (
    <section className="border-border bg-surface mb-3 overflow-hidden rounded-md border">
      <header className="border-border bg-bg/60 border-b px-3 py-1.5 font-mono text-[11px]">
        ...
        <span className="text-text-muted ml-2">{drafts.length} {drafts.length === 1 ? 'comment' : 'comments'}</span>
      </header>
...
function Snippet({ draft, snapshot }: { draft: ReviewDraft; snapshot: ReturnType<typeof useFileSnapshot> }): JSX.Element {
  if (snapshot.kind === 'loading' || snapshot.kind === 'idle') {
    return <div className="text-text-muted py-2 text-[11px]">Loading snippet…</div>
  }
  if (snapshot.kind === 'error') {
    return <div className="text-text-danger py-2 text-[11px]">Snippet unavailable: {snapshot.message}</div>
  }
  const snap = snapshot.snap
  if (snap.encoding !== 'utf8' || !snap.content) {
    return <div className="text-text-muted py-2 text-[11px]">Snippet unavailable ({snap.encoding}).</div>
  }
...
function CommentBody({ body, side }: { body: string; side: ReviewDraft['side'] }): JSX.Element {
  return (
    <div className="border-border bg-bg rounded-sm border p-2">
      ...
      <p className="text-text-primary text-xs whitespace-pre-wrap break-words leading-relaxed">{body}</p>
    </div>
  )
}

function Footer({ submitting, error, count, event, onCancel, onSubmit }: {
  submitting: boolean
  error: string | undefined
  count: number
  event: ReviewEvent
  ...
  return (
    <div className="border-border bg-surface flex shrink-0 items-center justify-between gap-3 border-t px-4 py-3">
      {error
        ? <span className="text-text-danger flex-1 truncate text-xs" title={error}>{error}</span>
        : <span className="text-text-muted flex-1 text-xs">Review will be posted to GitHub as a single batch.</span>}
