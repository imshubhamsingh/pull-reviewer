...
export function RefChips({ refs, onClick }: Props): JSX.Element {
  ...
  return (
    <div className="bg-bg/40 flex flex-wrap items-center gap-1.5 rounded-md px-2 py-1.5">
      <span className="text-text-muted shrink-0 text-[11px] tracking-wider uppercase">refs</span>
      {refs.map((ref, i) => (
        <button
          ...
          className={cn(
            ...
            'inline-flex max-w-full items-center gap-1 truncate rounded-sm px-2 py-0.5 font-mono text-[12px] transition-colors',
          )}
        >
