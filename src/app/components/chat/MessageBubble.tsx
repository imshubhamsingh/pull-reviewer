...
import { RefChips } from '@/app/components/chat/RefChips'
 ...
 */
export function MessageBubble({ message, onDelete, onJumpRef, onUseAsComment }: Props): JSX.Element {
  const isUser = message.role === 'user'
  const refs = !isUser && message.references && message.references.length > 0 ? message.references : null
  const canUseAsComment = !isUser && refs != null && message.status === 'complete' && onUseAsComment != null
  return (
    <div className={cn('flex w-full px-3 py-2', isUser ? 'justify-end' : 'justify-start')}>
      <div className={cn('group flex min-w-0 max-w-[90%] flex-col gap-1.5', isUser && 'order-2')}>
        ...
        {canUseAsComment && (
          <button
            type="button"
            onClick={() => { void onUseAsComment(message) }}
...
function Header({ message, onDelete }: { message: PrChatMessage; onDelete?: (id: number) => void }): JSX.Element {
  ...
  return (
    <div className="text-text-muted mb-1 flex items-center justify-between gap-2 text-[10px] tracking-wider uppercase">
      <span>{tone}{statusSuffix(message.status)}</span>
  ...
  return (
    ...
    >
      {isUser
        ...
        : <div className="markdown min-w-0 break-words" dangerouslySetInnerHTML={{ __html: renderedMarkdown }} />}
    </div>
  )
}
