...
import { QaThreadCard } from '@/app/components/QaThreadCard'
 ...
 */
export function DocsPane({ step, chapter, qaThreads, tourFilePaths, onDeleteQa, onJumpToRef }: Props): JSX.Element {
  ...
  return (
    <article className="markdown h-full overflow-y-auto p-5">
      ...
      {qaThreads.length > 0 && (
        <div className="mt-2">
          {qaThreads.map((t) => <QaThreadCard key={t.id} thread={t} onDelete={onDeleteQa} />)}
        </div>
      )}
    </article>
  )
