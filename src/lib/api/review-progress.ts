...
export const reviewProgress = {
  chapters: {
    list: (repo: string, prNumber: number, headSha: string) =>
      http.get<ChapterCompletion[]>(`/api/review-progress/${repo}/${prNumber}/${encodeURIComponent(headSha)}/chapters`),
    mark: (repo: string, prNumber: number, headSha: string, chapterId: string) =>
      http.post<ChapterCompletion>(`/api/review-progress/${repo}/${prNumber}/${encodeURIComponent(headSha)}/chapters/${encodeURIComponent(chapterId)}`),
    unmark: (repo: string, prNumber: number, headSha: string, chapterId: string) =>
      http.del<{ deleted: boolean }>(`/api/review-progress/${repo}/${prNumber}/${encodeURIComponent(headSha)}/chapters/${encodeURIComponent(chapterId)}`),
  },
  files: {
    list: (repo: string, prNumber: number, headSha: string) =>
      http.get<FileReview[]>(`/api/review-progress/${repo}/${prNumber}/${encodeURIComponent(headSha)}/files`),
    ...
    markMany: (repo: string, prNumber: number, headSha: string, filePaths: string[]) =>
      http.post<FileReview[]>(`/api/review-progress/${repo}/${prNumber}/${encodeURIComponent(headSha)}/files`, { filePaths }),
    unmark: (repo: string, prNumber: number, headSha: string, filePath: string) =>
      http.del<{ deleted: boolean }>(`/api/review-progress/${repo}/${prNumber}/${encodeURIComponent(headSha)}/files/${encodeURIComponent(filePath)}`),
  },
}
