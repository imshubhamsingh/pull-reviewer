...
Output a fresh, complete JSON array that matches the schema exactly. Common pitfalls: missing `body` on a step (every step needs prose, even diagram steps), referencing files or line numbers not in the diff, wrong `panel`/`code` or `panel`/`diagram` pairing, JSON not wrapped in an array, **dropping files from the diff (every file MUST be pinned by some step's `code.file` OR appear in some step's `references[]`)**. Do not include any prose, markdown fences, or commentary — only the JSON array.
